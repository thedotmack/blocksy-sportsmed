...
function location_photos_meta_box_cb($post)
{
  ...
  $existinPhotos = get_post_meta($post->ID, 'google_place_photos', true);
  ...
  if (!empty($photos)) {
    ...
    echo "<img src='$primary_photo_url' style='width: 100%; height: auto; margin-bottom: 10px;'>";
...
function display_address_and_phone()
{
  global $post;
  ...
  if (empty($placeDetails)) {
    return '<p>No address or phone number available.</p>';
  }

  $formatted_address = $placeDetails->formatted_address ?? null;
  $formatted_phone_number = $placeDetails->formatted_phone_number ?? null;

  if ($formatted_address === null || $formatted_phone_number === null) {
    return '<p>No address or phone number available.</p>';
  }

  return '<p>Address: ' . esc_html($formatted_address) . '</p>'
    . '<p>Phone: ' . esc_html($formatted_phone_number) . '</p>';
}


/**
 * Fetch Google Place data and update post meta when post is saved.
 *
 * @param int     $post_id The post ID.
 * @param WP_Post $post    The post object.
 * @param bool    $update  Whether this is an existing post being updated or not.
 */
function save_google_place_data($post_id, $post, $update)
{
  ...
  // Get the place ID
  $placeId = get_field('place_id', $post_id);
  if (!$placeId) {
    return;
  }

  // Fetch and save place details
  $googlePlaceDetails = new GooglePlaceDetails($placeId, $post_id);
  $placeDetails = $googlePlaceDetails->updatePlaceDetails();

  // Fetch and save place photos
  $googlePlacePhotos = new GooglePlacePhotos($placeId, $post_id);
  $photos = $googlePlacePhotos->getPhotos();
  if (!is_wp_error($photos)) {
    update_post_meta($post_id, 'google_place_photos', $photos);
  }
}
add_action('save_post', 'save_google_place_data', 10, 3);

function add_google_places_meta_box()
{
  add_meta_box(
    'google_places_meta_box', // id
    'Google Places', // title
    'display_google_places_meta_box', // callback
    'wpseo_locations' // page
  );
}
add_action('add_meta_boxes', 'add_google_places_meta_box');

function display_google_places_meta_box($post)
{
  wp_nonce_field(basename(__FILE__), "meta-box-nonce");

?>
  <div>
    <label for="meta-box-text">Google Place</label>
    <input id="google_places_input" name="google_places_input" type="text">
    <input id="google_places_id" name="google_places_id" type="hidden">
  </div>
<?php
}
...
function save_custom_meta_box($post_id, $post, $update)
{
  ...
  if (isset($_POST["google_places_id"])) {
    $google_places_id = $_POST["google_places_id"];
  }
  update_post_meta($post_id, "google_places_id", $google_places_id);
}
add_action("save_post", "save_custom_meta_box", 10, 3);

function enqueue_google_maps()
{
  wp_enqueue_script('google-maps', 'https://maps.googleapis.com/maps/api/js?key=' . GOOGLE_API_KEY . '&libraries=places', array(), '1.0', true);
  wp_enqueue_script('google-maps-init', get_stylesheet_directory_uri() . '/js/google-maps-init.js', array('google-maps'), '1.0', true);
}
